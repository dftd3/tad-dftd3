...
from the s-dftd3 Fortran *library*, at the cutoff
``tad_mctc.ncoord.defaults.CUTOFF_D3``. Unlike the energy/gradient/Hessian
references in the other ``test_*`` packages, these values are not computed
live at test time (that would make every test run depend on a Fortran
compiler and a from-source s-dftd3 build) -- they are dumped once and
committed here instead. See ``tools/refs/README.md`` for what generated
them and how to regenerate them.
"""
...
class Refs(TypedDict):
    ...
    cn: Tensor
    """Coordination number, shape ``(nat,)``."""

    weights: Tensor
    """
    Reference-system weights, shape ``(nat, 7)``.

    s-dftd3's Fortran ``weight_references`` only allocates as many
    reference-system slots as the elements in one structure actually need
    (``mref = maxval(disp%ref)``); tad-dftd3 always allocates a fixed 7
    slots per element (see ``tad_dftd3.reference.Reference``), zero-padding
    unused ones. The values below are already zero-padded out to width 7 so
    that they compare directly against tad-dftd3's own output.
    """

    c6: Tensor
    """Atomic C6 coefficients, shape ``(nat, nat)``."""
